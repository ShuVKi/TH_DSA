...
void removeAfter(List *&L, int val)
{
    ...
    if (!temp|| !temp->p_next) return;
    NODE* del = temp->p_next;
    temp->p_next = del->p_next;
    if (del == L->p_tail) L->p_tail = temp;
    delete del;
}
...
bool addPos(List*& L, int data, int pos)
{
    ...
    if (curr == nullptr) L->p_tail = newNode;
    return true;
}
...
void removePos(List *&L, int data, int pos)
{
    ...
    {
        prev->p_next = curr->p_next;
        if (curr == L->p_tail) L->p_tail = prev;
        delete curr;
    }
}
...
bool addBefore(List *&L, int data, int val)
{
    ...
    NODE* temp = L->p_head;
    while (temp->p_next && temp->p_next->key != val)
    {
        temp = temp->p_next;
    }
    NODE* newNode = createNode(data);
    if (!newNode || !temp->p_next) return false;
...
bool addAfter(List *&L, int data, int val)
{
    ...
    if (temp == L->p_tail) L->p_tail = newNode;
    return true;
}
...
bool removeElement(List*& L, int key)
{
    ...
    NODE* temp = L->p_head;
    while (temp->p_next && temp->p_next->key != key)
    {
        temp = temp->p_next;
    }

    if (temp->p_next == nullptr) return false;

    if (temp->p_next == L->p_tail) L->p_tail = temp;
    NODE* del = temp->p_next;
