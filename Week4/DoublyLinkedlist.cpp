...
void removeTail(d_List *&L)
{
    if (!L || !L->p_tail) return;
    d_NODE* temp = L->p_tail;
    L->p_tail = L->p_tail->p_prev;
    if (L->p_tail) L->p_tail->p_next = nullptr;
...
void removeBefore(d_List*& L, int val)
{
    if (!L || !L->p_head || L->p_head->key == val) return;
    d_NODE* curr = L->p_head->p_next;
    while (curr && curr->key != val)
    {
        curr = curr->p_next;
    }
    if (!curr||!curr->p_prev) return; //khong co node co gia tri val
    d_NODE* del = curr->p_prev;
    if (del == L->p_head) removeHead(L);
    else
    {
        del->p_prev->p_next = curr;
        curr->p_prev = del->p_prev;
        delete del;
    }
}
...
void removeAfter(d_List *&L, int val)
{
    if (!L || !L->p_head || L->p_head==L->p_tail) return;//L = nullptr || L rong || L co 1 NODE
    ...
    if (!curr|| !curr->p_next) return; //val is NODE cuoi || khong co val

    d_NODE* del = curr->p_next;
    if (del == L->p_tail) removeTail(L);
    else
    {
        curr->p_next = del->p_next;
        del->p_next->p_prev = curr;
        delete del;
    }
}
...
bool addPos(d_List*& L, int data, int pos)
{
    ...
    {
        curr = curr->p_next;
        if (curr == nullptr && i < pos) return false;
    }
    if (curr == nullptr) return addTail(L, data);
    else
    {
        d_NODE* newNode = createNode(data);
        if (!newNode) return false;
...
{
    if (!L || !L->p_head) return false;
    if (L->p_head->key == val) return addHead(L, data); // val is NODE dau

    d_NODE* temp = L->p_head->p_next;
...
bool addAfter(d_List*& L, int data, int val)
{
    ...
    if (temp == L->p_tail) return addTail(L, data);
...
void removeDuplicate(d_List *&L)
{
    ...
    while (temp)
    {
        d_NODE* curr = temp->p_next;
        while (curr)
        {
            if (curr->key == temp->key)
            {
                d_NODE* prevcurr = curr->p_prev;
                if (curr == L->p_tail) removeTail(L);
                else
                {
                    curr->p_prev->p_next = curr->p_next;
...
{
    if (!L || !L->p_head) return false;
    if (L->p_head->key == key) return removeHead(L);
